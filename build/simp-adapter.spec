...
# and thus more capabile Ruby than is provided by the OS (esp. on el6)
...
(
  if [ $1 -eq 0 ]; then
    ...
    # used by simp_rpm_helper.  To be sure we have all artifacts, we
    # will remove the default directory and the configured directory.
    export SIMP_ADAPTER_WORK_DIR=`/opt/puppetlabs/puppet/bin/ruby -r yaml -e "x = YAML.load(File.read('/etc/simp/adapter_conf.yaml')); puts (x && x.key?('work_dir')) ? x['work_dir'] : ''"`
